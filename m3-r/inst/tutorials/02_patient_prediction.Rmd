...
The donor predictor summarises each donor as a cell-type-balanced mean of the
latent embedding, then removes batch effects from it by adversarial domain
adaptation: a **corrector** reshapes the profile while a **batch discriminator**
tries to identify the batch from it, and the corrector is trained to predict
disease while preventing the discriminator from succeeding. The `donor_predictor`
settings:
